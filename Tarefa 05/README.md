...
A primeira coisa a fazer é fazer a conversão do **sentimento** de string para float e criar um novo atributo `sentiment_to_rows` que será ordenado do menor para o maior sentimento:
...
for row in self.rows:
      if row[8] != '':
        ...
        row[8] = 0.0
    self.sentiment_to_rows = sorted(self.rows, key=lambda r: r[8])
```

Utilizamos da função lambda para fazer uma função rápida para pegar os valores dos sentimentos.

Agora precisamos fazer uma busca binária, afim de conseguirmos uma área das mensagens, chamaremos este método de `get_message_range`:

```
def get_message_range(self, sentiment, rs=0):
    start = rs
    end = len(self.sentiment_to_rows) - 1

    while start < end:
      mid = (end + start) // 2
      value = self.sentiment_to_rows[mid][8]
      if value == sentiment:
        return mid
    ...
    if self.sentiment_to_rows[start][8] != sentiment:
      return -1
    return start
```

Com isto, vamos para o método principal, `get_rows_by_sentiment`:

```
def get_rows_by_sentiment(self, inf, sup):
    inferior = self.get_message_range(inf,0)
    if inferior == -1:
      return -1

    superior = self.get_message_range(sup, inferior)
    if superior == -1:
      return -1

    rows = self.sentiment_to_rows[inferior:superior + 1]
...
Para este fim, criamos o método `sum_scores`:

```
def sum_scores(self, target_score):
    for i in range(len(self.rows)):
      j = i + 1
      for j in range(len(self.rows)):
        if int(self.rows[i][9]) + int(self.rows[j][9]) == target_score:
          return self.rows[i], self.rows[j]
    return None
```

Neste método utilizamos de um for duplo para percorrer as linhas afim de somar os **scores** e caso seja igual ao **score_alvo** as linhas aonde os alvos.

Porém, precisamos reduzir a complexidade deste algoritmo. Com isto precisamos de um novo atributo, que chamaremos de `scores` que será um dicionário que receberá a linha como conteúdo e sua chave será o **score** que será convertido em inteiro, afim de facilitar o código:

```
self.scores = {}
for row in self.rows:
    self.scores[int(row[9])] = row
```

E implantaremos o método `sum_scores_fast` que percorrerá todo o dicionário, pegaremos o **score_alvo** e substraímos da chave atual do loop e verificamos se o resultado desta operação está no dicionário, caso positivo, retornar as linhas da chave e do resultado.

```
def sum_scores_fast(self, target_score):
    for key in self.scores:
      result = target_score - key
      if result in self.scores:
        return self.scores[key], self.scores[result]
    return None
```
...
import random
import pytest
filename = '/content/drive/MyDrive/EngComp/Estrutura de Dados II/Fast Queries/the-reddit-climate-change-dataset-comments.csv'
